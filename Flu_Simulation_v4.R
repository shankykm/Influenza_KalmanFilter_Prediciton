...
# Synthetic Weekly means and standard deviations for P and T
mean_P <- c(
...
# =============================================================================
# Extract historical priors Model Definition
...
model {
  ...
  # non informative priors for ensemble weight
  w1 ~  dunif(0, 1)
...
# =============================================================================
set.seed(100)
h=2  # set a specific starting week for your forecast. h=50 will start predicting from week 3 onwards
n_ahead <- 4
...
for (m in modelname){
  kf_multi_step_ensem_inforPrior_predict <- flu_data %>%
    ...
    do(kf_ensem_predict_informPrior(., kf_ensem_informativePriors_agg_df,
                                    var1, var2, "covar1",
...
set.seed(100)
h=2 # set a specific starting week for your forecast. h=50 will start predicting from week 3 to 52
n_ahead <- 4 
nc <- 3 # MCMC chain
...
simul_combined_df$dep_var <- factor(simul_combined_df$dep_var,
                                    levels = c("total_vol", "Detected"))
...
ggplot(summary_metrics, aes(x = factor(forecast_step), y = percent_within_CI, fill = model)) +
  ...
  geom_text(data=summary_metrics_stats, aes(as.factor(forecast_step),
                                            round(mean_percent_within_CI,2), label= round(mean_percent_within_CI,2)),
            position = position_dodge(width = 0.75), size = 2.5, vjust = -0.5)+
  facet_wrap(~dep_var, scales = "free") +
  theme_minimal() +
  theme(legend.position = "bottom")
